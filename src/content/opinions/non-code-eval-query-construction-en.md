---
title: "How to Build High-Quality Queries for Non-Code Evaluation Sets"
lang: "en"
translationKey: "non-code-eval-query-construction"
date: 2026-05-29
summary: "A practical guide to building verifiable, attributable, and evolvable eval queries for non-code text tasks."
authors:
  - name: "Xinhui Huang"
    github: "ivyandbonjuice"
stance: "The hard part of non-code text evaluation is not only Judge scoring, but that queries often fail to structure the real task, contextual dependencies, hard constraints, and failure modes, making model capability boundaries hard to observe."
...
The core problem with L1 is that as long as the model is fluent and well-structured, it can easily get a high score. But this only tests whether the model can summarize; it cannot distinguish models that truly have planning and judgment capabilities.
...
A simple test is: after removing all environmental materials, can the model still complete the key judgment? If the answer is almost the same after removing the materials, the query is probably L1/L2. If the key judgment becomes impossible without the materials, it is truly L3.
...
## 2. Comparing mainstream task-construction patterns

Public materials rarely disclose full query synthesis pipelines, but GDPval, Anthropic agent evals, Kimi Claw Bench, and LangSmith / AgentEvals reveal a shared set of construction dimensions.

Here, ☑️ means the element is clearly reflected in public materials, ◐ means it is partially reflected or reasonably inferable, and ❌ means I did not find an obvious public design. The coverage score only indicates observable task/query construction elements in public materials; it is not a ranking of overall quality.

| Construction dimension | OpenAI GDPval | Anthropic Agent Evals | Kimi Claw Bench | LangSmith / AgentEvals |
| --- | ---: | ---: | ---: | ---: |
| Based on real workflows, not exam-style questions | ☑️ | ☑️ | ☑️ | ☑️ |
| Tasks include reference files / environment | ☑️ | ☑️ | ◐ | ☑️ |
| Tasks are expert-crafted or expert-reviewed | ☑️ | ☑️ | ◐ | ◐ |
| Covers multi-industry / multi-scenario taxonomy | ☑️ | ☑️ | ☑️ | ◐ |
| Focuses on final deliverable quality | ☑️ | ☑️ | ☑️ | ☑️ |
| Evaluates process traces / tool calls | ❌ | ☑️ | ☑️ | ☑️ |
| Supports multi-turn or long-horizon tasks | ❌ | ☑️ | ☑️ | ☑️ |
| Feeds production traces / bad cases back into datasets | ◐ | ☑️ | ◐ | ☑️ |
| Uses automatic graders / rubrics | ☑️ | ☑️ | ☑️ | ☑️ |
| Emphasizes regression testing and continuous iteration | ◐ | ☑️ | ◐ | ☑️ |
| Coverage score | 7.5 / 10 | 10 / 10 | 8 / 10 | 8.5 / 10 |

Several trends stand out.

First, mainstream task construction has clearly moved from "writing a prompt" toward "building a task package". OpenAI GDPval is a representative example: tasks are not simple text prompts, but real knowledge-work tasks with reference files, context, and explicit deliverables.

Second, Agent evaluations pay more attention to process traces. Anthropic and LangSmith both include transcripts, tool calls, and environment outcomes in the evaluation object. It is not enough for the model to say "I completed it"; the evaluation also needs to see whether it called the right tools, changed environment state, and recovered after failures.

Third, production data is becoming a major source of high-value queries. LangSmith's data flywheel is clear: discover issues from online traces, add bad cases to the dataset, and use them for later regression tests. Anthropic also emphasizes starting from real failed tasks rather than writing a large closed-door question bank from day one.

I therefore define the standard structure of a non-code eval query as:
...
If it only has the user task, it is still just a prompt. Once context, deliverables, checks, and failure modes are added, it starts to become an evaluation asset.

## 3. Where queries come from: five sources with different value and cost

High-quality queries should not be invented purely from imagination. They can come from five types of sources:
...
For cold start, I prefer this ratio:
...
Good queries often come from friction in real scenarios: users complain that "this cannot be used directly"; business stakeholders say "this is not our tone"; reviewers point out "there is a compliance risk here"; operations teams report that "the conversion point appears too late".
...
Difficulty ranges can be used this way:
...
| Query | Difficulty estimate | Why |
| --- | --- | --- |
| "Help me write a template for e-commerce after-sales communication." | 1 point | It barely depends on external materials and has no obvious goal conflict |
| "I bought a down jacket last week and it is leaking down. It has been more than 7 days, but it is still under warranty. I want a partial refund while keeping the item, and I also want to complain that the product page misrepresented the material. Please give me negotiation talking points, likely merchant objections, the platform intervention process, and an evidence checklist." | 7 points | It has constraints, conflict, and judgment, but still lacks verifiable environment |
| Same as above, but add order JSON, product-page snapshot, platform policy, and similar precedents; require risk levels and priority ranking; in the second turn, the user adds: "The merchant says the warranty does not cover human-caused damage. How should I respond?" | 10-11 points | It requires cross-verification across materials, multi-step planning, and multi-turn state tracking |
...
After synthesizing a query, do not rush it into the evaluation set. Run it through a checklist first.

| Basic validity | Passing standard |
| --- | --- |
...
| Structural completeness | What to check |
| --- | --- |
...
| Discriminative-power signal | Action |
| --- | --- |
...
Below is a non-code eval query suitable for inclusion. Every field has a reason to exist: `environment` prevents the model from answering only with general knowledge; `hard_checks` makes evaluation attributable; `failure_modes` gives direction for the next iteration.
...
environment:
  - order_detail.json
  - product_page_snapshot.html
  - platform_refund_policy.md
  - dispute_precedent.md
...
This kind of query is valuable not because it is long, but because it can test several key capabilities at once: rule understanding, missing-information handling, multi-goal conflict, risk judgment, workflow planning, and executable expression.
...
An executable data flywheel can be divided into six steps:
...
Maintenance can follow these signals:
