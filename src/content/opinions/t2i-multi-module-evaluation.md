...
ERNIE-Image 发布至今已有一段时间，围绕文本渲染、复杂指令遵循和真实可用性的反馈逐渐积累。

这篇文章讨论一套以 LLM-as-a-Judge 为核心的全自动评估系统：它试图**突破单一指标 benchmark 的局限**，更系统地**捕捉现代文生图模型的能力边界**，并通过诊断模型短板为后续迭代提供依据。
...
这套框架的核心价值在于：全面覆盖、鲁棒校准、自适应评分和可解释诊断。它能持续暴露模型短板，并为后续迭代提供可追溯的评估依据。
...
ERNIE-Image 模型策略产品经理，评测与数据组，百度。
